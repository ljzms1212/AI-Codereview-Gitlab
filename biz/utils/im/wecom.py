...
class WeComNotifier:
    ...
    def _get_webhook_url(self, project_name=None, url_slug=None):
        ...
        raise ValueError(
            f"未找到项目 '{project_name}' 对应的企业微信 Webhook URL，且未设置默认的 Webhook URL。"
        )
    ...
    def send_message(
        ...
        try:
            ...
            # 转换md 的时候，长度可能会溢出，偷懒办法是将文本大小留一个余量
            MAX_CONTENT_BYTES = 3600 if msg_type == "markdown" else 1800
            ...
            if content_length <= MAX_CONTENT_BYTES:
                ...
                # 内容超过限制，需要分割发送
                logger.warning(
                    f"消息内容超过{MAX_CONTENT_BYTES}字节限制，将分割发送。总长度: {content_length}字节"
                )
    ...
    def _send_message_in_chunks(
        ...
        for i, chunk in enumerate(chunks):
            chunk_title = (
                f"{title} (第{i + 1}/{len(chunks)}部分)"
                if title
                else f"消息 (第{i + 1}/{len(chunks)}部分)"
            )
    ...
    def _send_message(self, post_url, data, chunk_num=None, total_chunks=None):
        ...
        try:
            ...
            if response and response.get("errcode") != 0:
                logger.error(
                    f"企业微信消息发送失败! webhook_url:{post_url}, errmsg:{response}"
                )
            else:
                logger.info(
                    f"企业微信消息{'分块' if chunk_num else ''}发送成功! webhook_url:{post_url}"
                )
